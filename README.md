...
아래 명령은 모두 **리포 루트**(`Path_Planning_Core_/`, 이 README가 있는
위치)에서 실행한다고 가정한다. `-S .`는 "현재 디렉터리를 소스 루트로
써라", `-B build`는 "리포 루트 바로 아래에 `build/` 폴더를 만들고 그 안에
빌드 산출물을 넣어라"는 뜻이라, 다른 위치에서 실행하면 `-S`/`-B` 경로를
그에 맞게 바꿔줘야 한다(예: 리포 루트가 아닌 곳에서 실행하려면
`cmake -S <리포 루트 경로> -B build`처럼 절대/상대 경로를 직접 지정).
...
`run_tests`, `run_benchmark`, `mapf_py`는 모두 `mapf_core` 라이브러리를
링크하도록 각 `CMakeLists.txt`에 선언되어 있다. CMake는 이 의존 관계를
보고 **타겟을 빌드하기 전에 `mapf_core`가 최신 상태인지 자동으로 확인하고,
필요하면 알아서 먼저 빌드한다** — 즉 위 세 명령 중 아무거나 하나만
실행해도 되고, `mapf_core`를 미리 따로 빌드해둘 필요는 없다(다만
`mapf_core` 자체만 빌드하고 싶다면 `cmake --build build --target mapf_core
--config Release`로 가능하다).
...
`plot_benchmark.py`는 위치 인자(positional argument) 2개를 받는다(`--`로
시작하는 옵션이 아니라, 순서로 구분되는 인자다):

| 순서 | 인자 | 필수 여부 | 설명 |
|---|---|---|---|
| 1 | `csv_path` | 필수 | `run_benchmark`(위 단계)가 만든 CSV 파일 경로. 없으면 사용법 안내를 출력하고 종료한다. |
| 2 | `output_dir` | 선택(기본값 `tools/plots`) | PNG 4종을 저장할 폴더. 생략하면 스크립트가 있는 위치 기준 `tools/plots`에 저장된다. 폴더가 없으면 자동으로 생성한다. |
...
| **← / Backspace** | 한 스텝 되돌리기 — 마지막 SPACE 직전 상태로 복원한다. 그 시각 이후에 놓은 장애물과 재계획 결과도 함께 취소된다(나중에 생긴 장애물을 남겨두면 그 장애물이 원래보다 일찍 생긴 셈이 되어 시나리오가 달라지므로). 재계획 실패로 잠긴 상태에서도 동작한다. |
| **W** | "기다리는(양보 중인) 로봇" 강조 켜기/끄기(기본 켜짐). 이번 스텝에 제자리에 머물다가 나중에 다시 움직일 로봇의 칸 둘레에 주황 사각 테두리를 그리고, 하단 상태 바에 패널별 대기 로봇 수를 표시한다. 목적지에 도착해서 머무는 로봇은 제외. |
...
로봇들이 현재 위치에 멈추고 화면 하단에 "←로 되돌리거나 R키로 재시작하라"는
안내가 뜨며, 그동안은 클릭/SPACE 조작이 막힌다(←로 실패 직전으로 되돌려 다른
칸에 장애물을 놓아볼 수 있다).
