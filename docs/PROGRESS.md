...
## 규모 확장 실험 + 추적 단계(Tier) 깊이 비교 (2026-09-13)

### 동기

A* 수정 후 32x32 벤치마크는 두 방법 모두 성공률이 거의 100%라 성공률로는
차이가 안 났고, corridor 40대는 시간 이점도 없었다(Tier 0/1 성공 시 약 2.7배
빠르지만 안전망까지 가면 약 1.5배 느려 평균이 같아짐). 전체 재계획이 10ms대로
원래 빨라서 차이를 볼 여지가 없다고 보고 규모를 키웠다.

### 도구

- `run_benchmark --large`(64x64, 로봇 50/100/150/200대, 장애물은 그대로 1~3칸),
  `--repeats N`, `--tiers K`(`max_escalation_tiers`). 인자 없이 돌리면 기존
  결과와 시간 외 모든 칸이 동일함을 확인.
- `tools/plot_tiers.py`(K별 비교 그래프 3장), `plot_benchmark.py` 시간 축 눈금을
  데이터 범위 기반으로 수정(대규모 결과에서 50ms 위 눈금이 비던 문제).

### 결과 1: 추적 단계 1 (`tools/plots_large/`)

- Tier 0/1이 성공하면 모든 조건에서 전체 재계획보다 4~18배 빠르고, 안전망까지
  가도 손해는 5~10%뿐(32x32의 1.5배보다 작음 — Tier 시도 비용이 상대적으로 작아짐).
- 그래서 평균 이점은 Tier 0/1 성공률이 결정한다. 밀도가 높을수록 성공률이
  떨어진다(open 96%→44%, corridor 62%→14%). open 50~100대는 중앙값 5~6배,
  150대 이상은 1.1~1.6배, corridor 150대 이상은 약 1배.

### 결과 2: 추적 단계 1/2/3/5 비교 (`tools/plots_tiers/`, 같은 시나리오 400개)

| | 안전망 없이 해결(K=1→5) | 짝지은 속도 향상 중앙값(K=1→5) |
|---|---|---|
| open 100 | 74 → 96% | 6.7 → 10.2배 |
| open 150 | 52 → 80% | 1.9 → 4.5배(K=3에서 5.1배) |
| open 200 | 44 → 62% | 1.1 → 1.5배 |
| corridor 50 | 62 → 90% | 2.0 → 2.5배 |
| corridor 150~200 | 16~14 → 48~26% | 약 0.9배(변화 없음) |

- 단계 2~3에서 효과 대부분이 나오고 5는 추가 이득이 작다.
- corridor 200대: 선택적(K=5) 58% vs 전체 42% — 옛 경로를 유지하며 필요한
  로봇만 부르는 쪽이 모두를 다시 짜는 쪽보다 나은 경우가 있다(trial 36 분석 참고:
  전체 재계획에서는 앞 로봇들이 다른 최단 경로를 골라 뒤 로봇의 통로를 막음).
- 한계: corridor 150대 이상은 K와 무관하게 속도 이점이 없고, corridor 200대 K=5는
  오히려 느려짐(끝내 실패하는 경우 여러 단계를 헛돔).

### 주의 / 남은 과제

- 시간은 1회 측정이라 흔들린다(같은 시나리오 open 50 K=1이 실행마다 5.9배/8.0배).
  논문용으로는 반복 측정 필요. 전체 재계획이 수 초 걸리는 경우의 원인은 미분석.
- `07_research_positioning.md` 7.5절의 "40대 기준 약 20배"는 옛 Python 벤치마크
  수치라 현재 결과와 맞지 않음 — 갱신 필요.

---

## 알고리즘 검증 → 등록 순서 변경(fixed-first) → 아이디어 3 (2026-09-13)

쉬운 전체 정리는 [14장](14_session_summary_easy.md). 자세한 기록은 10~13장.

### 검증 (10·11장)

- `PBS` 클래스는 실제로는 고정 우선순위 계획(PP). 결과 충돌 논리 오류는 없음.
- 선택적 재계획 실패의 99% 이상이 "고정 로봇과 재계획 로봇을 우선순위 순서로 섞어 등록"한
  구조 때문. 임시 계측으로 확인: 거절돼 불려 온 구조 로봇은 거의 항상 비켜 가지만(197/197),
  시도가 첫 거절에서 멈춰 원래 있던 충돌이 하나씩만 드러나는 "숨은 충돌"이 실패의 약 75%.

### 등록 순서 변경 (12장)

- `PBSConfig::order` = `ReplanOrder::kPriority`(기본, 원래) / `kFixedFirst`(고정 로봇 먼저 →
  처음 영향받은 로봇 → 구조 로봇). `try_replan_set`은 처리 순서만 바꿈.
- 대규모 안전망 없이 성공 207 → 394/400, 경로가 바뀐 로봇 수 평균 45.4 → 4.5. 우선순위
  높은 로봇이 돌아가는 대가(비용 최대 +24)가 있지만 평균 비용은 감소. 충돌 0/3117.
- 벤치마크 `--order`, 경로 품질 컬럼(`initial_soc`, `*_soc`, `*_changed`,
  `selective_replanned`, `selective_conflicts`) 추가.

### 아이디어 3 (13장)

- `PBSConfig::check_reachability`: 벽+새 장애물만으로 BFS, 못 가면 즉시 실패(결과 동일, 시간만 절약).
- `PBSConfig::rescue_selection` = `kBlockedCells`(기본) / `kShortestPathConflicts`(최단 경로와
  부딪히는 로봇만 구조 후보).
- 벤치마크 `--reachability`, `--rescue` 추가. 테스트 총 52개.
- 대규모(fixed-first 위, Tier 1): 3-2로 안전망 3→0, 실패 3→2, 다시 계획한 로봇 수 최댓값
